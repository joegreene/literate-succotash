...
// used for the multithreading portions
using std::async;
using std::future;
...
int main()
{
  ...
  // steps: treat the total_sum as an unsigned long*, dereference that (to prevent any implicit casts from double to long)
...
void threaded_compute(double& total_sum, double& last_term, unsigned long num_terms) {
  future<double> thread_list[NUM_THREADS];                   // List of threads (where each thread returns a double value)
  int            i,j;                                        // your everyday run-of-the-mill loop control variables
  unsigned long  term_length    = num_terms/NUM_THREADS;     // 
  double*        last_term_list = new double[NUM_THREADS];   // 
  ...
  // free memory used in the term list work
  delete [] last_term_list;
}
